...
static LtBool CRC16(LtByte *aBuffer, LtByte aLength, LtBool aCheck)
{
	...
	if (aCheck)
	{
		//printf("CRC expected: %02Xh %02Xh\nCRC received: %02Xh %02Xh\n", (unsigned int)lCRCHi, (unsigned int)lCRCLo, (unsigned int)aBuffer[aLength + 1], (unsigned int)aBuffer[aLength]);
...
{
	return pConnection->handle != LT_INVALID_HANDLE;
}
...
LtResult ModbusSend(SModbusInfo* pConnection, LtByte aFunction, LtByte *aBuffer, LtByte aLength)
{
	...
	if ((aLength > MODBUS_MAX_PAYLOAD) || (aFunction < 1) || (aFunction > 127))
	{
		return LT_INVALID_ARGUMENT;
	}

	lMessage[0] = pConnection->address;
	...
	if (WriteToSerialPort(pConnection->handle, lMessage, aLength) == aLength)
	{
		pConnection->lastFunction = aFunction;
		return LT_SUCCESS;
	}
...
LtResult
ModbusReceive(SModbusInfo* pConnection, LtByte *aBuffer)
{
	LtResult lRead;
	LtByte   lMessage[MODBUS_MAX_LEN];

	lRead = ReadFromSerialPort(pConnection->handle, lMessage, sizeof(lMessage));
	...
	if (lRead >= MODBUS_OVERHEAD)
	{
		...
		{
			if ((lMessage[0] == pConnection->address) && 
				(lMessage[1] == pConnection->lastFunction))
			{
				pConnection->lastFunction = 0;
				if (aBuffer != NULL)
				{
...
LtResult ModbusConnect(SModbusInfo** ppConnection, const char *sPortName, LtByte aAddress,
					   unsigned int uBaudRate, unsigned int uParity, unsigned int uStopBits)
{
	if (ppConnection == NULL) return LT_ERROR;                         // Can't return the connection handle

	*ppConnection = (SModbusInfo*)malloc(sizeof(SModbusInfo));
	if (*ppConnection == NULL) return LT_ERROR;						   // Insufficient memory

	memset(*ppConnection, 0, sizeof **ppConnection);

	LtResult lResult = OpenSerialPort(sPortName, &(*ppConnection)->handle, 
									  uBaudRate, uParity, uStopBits);
	if (lResult == LT_SUCCESS)
	{
		(*ppConnection)->address = aAddress;

		lResult = ModbusSend(*ppConnection, MODBUS_SERVER_ID, NULL, 0);

		if (lResult == LT_SUCCESS)
		{
			LtByte lDummy[MODBUS_MAX_PAYLOAD];

			lResult = ModbusReceive(*ppConnection, lDummy);
	...
	CloseSerialPort((*ppConnection)->handle);
...
ModbusDisconnect(SModbusInfo* pConnection)
{
	if (pConnection != NULL)
	{
		CloseSerialPort(pConnection->handle);
		free(pConnection);
	}
}
...
LtResult
ModbusReadInputRegisters(SModbusInfo* pConnection, LtU16 aNo, LtU16 aCount, LtU16 *aValues)
{
	...
	lResult = ModbusSend(pConnection, 4, lPayload, 4);

	if (lResult == LT_SUCCESS)
	{
		lResult = ModbusReceive(pConnection, lPayload);
...
LtResult
ModbusReadHoldingRegister(SModbusInfo* pConnection, LtU16 aNo, LtU16 *aValue)
{
	LtByte   lPayload[MODBUS_MAX_PAYLOAD];
	LtResult lResult;
	...
	lResult = ModbusSend(pConnection, 3, lPayload, 4);
...
LtResult
ModbusWriteRegister(SModbusInfo* pConnection, LtU16 aNo, LtU16 aValue)
{
	LtByte   lPayload[MODBUS_MAX_PAYLOAD];
	LtResult lResult;
	...
	lResult = ModbusSend(pConnection, 6, lPayload, 4);
