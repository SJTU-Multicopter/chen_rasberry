...
static void SetNonBlocking( LtBool aState )
{
...
///
/// \param   sPortName  Name of the device to open (must be valid for the
///                     platform).
...
{
#ifdef LT_WINDOWS
	wchar_t      lPortName[LT_MAX_PORT_NAME_LEN];
	DCB          lDCB;
	COMMTIMEOUTS lCTO;
	...
	*aHandle = CreateFile(lPortName,
	...
	lCTO.ReadIntervalTimeout = 25;				   //25
	lCTO.ReadTotalTimeoutMultiplier = 0;		   //0
	lCTO.ReadTotalTimeoutConstant = 1000;		   //1000
	lCTO.WriteTotalTimeoutMultiplier = 10;		   //10
	lCTO.WriteTotalTimeoutConstant = 1000;		   //1000
...
#endif // LT_WINDOWS
#ifdef LT_LINUX
	char lPortName[LT_MAX_PORT_NAME_LEN];

	sprintf( lPortName, "/dev/%s", sPortName );
...
LtResult ReadFromSerialPort(LtHandle aHandle, LtByte *aData, int aMaxLength)
{
	...
	while( lRead < aMaxLength )
	{
		...
		lTimeout.tv_sec = 0;
		lTimeout.tv_usec = 2000;        // STR: Was 20000000/9600 in the worst case, so keeping it 2000us all the time should be ok!
